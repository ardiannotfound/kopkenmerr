...
export default function DetailIncidentScreen() {
  ...
  const [assetName, setAssetName] = useState(isQrScan && assetId ? assetId : '');
  const [date] = useState(new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })); 
  const [title, setTitle] = useState(isQrScan ? `Laporan Aset: ${assetId}` : '');
  ...
  const handleSubmit = () => {
    ...
    Alert.alert(
      'Berhasil', 
      'Tiket Pengaduan berhasil dikirim! Nomor tiket akan dikirimkan ke email Anda.',
    ...
    <View style={styles.inputContainer}>
      ...
      <View style={[
        ...
        { 
          ...
          borderColor: isDarkMode ? '#444' : '#CBCBCB' 
        }
      ]}>
        <TextInput
          style={[
  ...
  const renderDropdown = (label: string, value: string, placeholder: string, icon: any = 'chevron-down') => (
    <View style={styles.inputContainer}>
      ...
      <TouchableOpacity style={[
        ...
        <Text style={{ 
          color: value ? (isDarkMode ? '#FFF' : '#333') : (isDarkMode ? '#666' : '#B0B0B0'), 
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
            ...
            <View style={styles.stepContainer}>
               <View style={styles.stepWrapper}>
                ...
                <Text style={styles.stepLabel}>Data Diri</Text>
              </View>
            ...
            <View style={[styles.reviewBox, { backgroundColor: isDarkMode ? '#2C2C2C' : '#F0F7FF', borderColor: isDarkMode ? '#444' : '#E5E2E2' }]}>
              <View style={{flexDirection:'row', alignItems:'center', marginBottom: 5}}>
                <Ionicons name="person-circle" size={20} color={isDarkMode ? '#AAA' : '#053F5C'} style={{marginRight:5}} />
                <Text style={[styles.reviewTitle, { color: isDarkMode ? '#FFF' : '#053F5C' }]}> Pelapor:</Text>
              </View>
              <Text style={[styles.reviewText, { color: isDarkMode ? '#CCC' : '#333' }]}>{userData?.name}</Text>
              <Text style={[styles.reviewSubText, { color: isDarkMode ? '#999' : '#666' }]}>{userData?.email} • {userData?.phone}</Text>
            </View>

            <View style={styles.sectionHeaderBox}>
              <Text style={styles.sectionDividerText}>Detail Pengaduan</Text>
            </View>

            <View style={styles.formGroup}>
              {renderDropdown('Nama OPD', opd, 'Pilih Dinas / OPD')}
              {renderDropdown('Jenis Layanan', serviceType, 'Pilih Jenis Layanan')}
              
              {isQrScan 
                ? renderDropdown('Nama Aset', assetName, 'Aset Terdeteksi', 'qr-code')
                : renderDropdown('Nama Aset', assetName, 'Pilih Aset Bermasalah')
              }
              
              {renderDropdown('Tanggal Kejadian', date, date, 'calendar')}

              {renderInput('Judul Pengaduan', title, setTitle, 'Contoh: Wifi Mati Total')}
              {renderInput('Deskripsi Pengaduan', description, setDescription, 'Jelaskan detail masalah...', true)}

              {/* UPLOAD BOX */}
              <Text style={[styles.label, { color: isDarkMode ? '#AAA' : '#053F5C' }]}>Lampiran (Opsional)</Text>
              <TouchableOpacity style={[styles.uploadBoxLarge, { backgroundColor: isDarkMode ? '#2C2C2C' : '#F0F9FF', borderColor: isDarkMode ? '#444' : '#053F5C' }]}>
                <Ionicons name="cloud-upload-outline" size={40} color={isDarkMode ? '#AAA' : '#555657'} />
                <Text style={styles.uploadTitleBlue}>Unggah Bukti</Text>
                <Text style={styles.uploadSubGray}>PNG, JPG, PDF (Maks. 5 MB)</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.footer}>
              ...
              <TouchableOpacity style={styles.btnSubmit} onPress={handleSubmit}>
                ...
                <Text style={styles.btnSubmitText}>Kirim Pengaduan</Text>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  textInput: {
    fontFamily: 'Poppins_400Regular', fontSize: RFValue(14), height: '100%'
  },
